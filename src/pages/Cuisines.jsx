...
const Cuisines = () => {
  ...
  useEffect(() => {
    ...
    if (!Object.values(activeFilters).some(Boolean)) {
      setFilteredRecipes(cuisines);
      return;
    }
    
    // Apply filters
    const filtered = cuisines.filter(recipe => {
      // Gluten Free filter
  ...
  const getCuisine = async (name) => {
    ...
    try {
      const api = await fetch(
        `https://api.spoonacular.com/recipes/complexSearch?apiKey=${process.env.REACT_APP_API_KEY}&number=12&cuisine=${name}&addRecipeInformation=true`
      );
    ...
    <Container>
      ...
      {isLoading ? (
        <LoadingContainer>
          ...
          />
          <LoadingText>Loading {params.type} recipes...</LoadingText>
        </LoadingContainer>
        ...
        >
          ...
          <NoResultsText>No {params.type} recipes match your current filters</NoResultsText>
